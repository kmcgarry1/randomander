...
- Randomander is a lightweight browser experience that uses live Scryfall data to find Commander-legal cards and compatible pairings, with optional EDHREC context.
...
3. **Add context without burying the cards** — Treat EDHREC counts and themes as optional inspiration after the result, never as a blocker for the core reveal.
...
- Choice mode makes two ideas genuinely comparable, including independent pair metadata rather than a combined details block.
...
Randomander is an inspiration tool, not a deck builder or Magic rules engine. It currently has no account, cloud sync, sharing/export workflow, repository-owned backend, or guaranteed offline draw path. Scryfall and EDHREC availability and schemas remain external dependencies.
