...
Use this guide whenever collaborating on `randomander`, especially during multi-person or AI-assisted work. See [CONTRIBUTING.md](CONTRIBUTING.md) for the complete development workflow and [docs/architecture.md](docs/architecture.md) for code ownership.

## Communication
- Reference `AGENTS.md` before AI-assisted repo work—system/developer instructions go first, then repository guidance.
...
- Run `npm run dev` for interactive debugging, `npm run test` for Vitest, and `npm run build` (Vue TSC + Vite) before shipping changes; report any blocked checks.
...
- Keep Tailwind utility usage consistent with the existing theme and Material-inspired token layer.
